...
if (isset($_GET['lpj_id']) && (get_status_lpj($_GET['lpj_id']) == '2' || get_status_lpj($_GET['lpj_id']) == '3')) {
	...
	$query = "SELECT * FROM TB_PENANDATANGAN_LJP WHERE LPJ_ID='$lpj_id'";
	$result = mysql_query($query);
	...
	<div style="border:4px solid black; border-radius: 40px; padding: 10mm; height: 100%">
		<table width="100%">
			<tbody>
				...
				<tr>
					<td style="padding-top: 20px; font-size: 140%; font-weight: bold;">PEMERINTAH KABUPATEN LEBAK</td>
				</tr>
				<tr>
					<td style="font-size: 140%; font-weight: bold;">DINAS KOMUNIKASI DAN INFORMATIKA</td>
				</tr>
				<tr>
					<td style="font-size: 9pt;">Jalan Abdi Negara No. 3 Tlp. (0252) 5552138<br>Rangkasbitung 42312 Lebak - Banten<br>email : diskominfo@lebakkab.go.id | website : diskominfo.lebakkab.go.id</td>
				</tr>
				<tr>
				...
				<tr>
					<td style="font-size: 140%; font-weight: bold;">KEGIATAN<br>'.$nama_kegiatan.'</td>
				</tr>
				<tr>
	...
	<div style="height: 100%">
		<table width="100%">
			<tbody>
				<tr>
					<th colspan="9">DINAS KOMUNIKASI DAN INFORMATIKA KABUPATEN LEBAK
						<br>
		...
		<table class="border" width="100%" style="border: 3px double black;">
			<thead >
				<tr>
					<th rowspan="2" width="5%">Kodering</th>
					<th rowspan="2" width="15%">Uraian</th>
					<th rowspan="2" width="7%">Jumlah<br>Anggaran</th>
					<th colspan="3" width="17%">LPJ - LS<br>Gaji</th>
					<th colspan="3" width="17%">LPJ - LS<br>Barang &amp; Jasa</th>
					<th colspan="3" width="17%">LPJ<br>UP/GU/TU</th>
					<th rowspan="2" width="10%">Jumlah<br>(LS+UP/GU/TU)</th>
					<th rowspan="2" width="10%">Sisa Pagu<br>Anggaran</th>
				</tr>
				<tr>
		...
		<table width="100%">
			<tbody>
				<tr>
					...
					<td>Rangkasbitung, '.$tgl_update.'</td>
				</tr>
				<tr>
	...
	while ($fetch = mysql_fetch_array($result)) { 
		...
		$buku_kas_umum['kode_rek'] = rtrim(chunk_split($fetch['BKU_KODE_REK']));
		$buku_kas_umum['uraian'] = $fetch['BKU_URAIAN'];
		$buku_kas_umum['atas_nama'] = $fetch['BKU_ATAS_NAMA'];
		...
		if ($fetch['BKU_ATAS_NAMA'] != NULL && $fetch['BKU_ATAS_NAMA'] != '') {	// Memiliki atas nama
			...
			$tabel_bku = $tabel_bku.'<td style="text-align: justify; font-size: 9pt;">'.$buku_kas_umum['uraian'].' atas nama '.$buku_kas_umum['atas_nama'].'</td>';
			...
			while ($fetch = mysql_fetch_array($result)) {
				...
				if ($fetch['BKU_URAIAN'] == $tmp_uraian) {
					$buku_kas_umum['no_urut'] = $fetch['BKU_NO_URUT'];
					$buku_kas_umum['tgl'] = date_format(date_create($fetch['BKU_TGL_SUB_KEG']), 'd/m/Y');
					$buku_kas_umum['kode_rek'] = rtrim(chunk_split($fetch['BKU_KODE_REK'], 1, '.'), '.');
					$buku_kas_umum['uraian'] = $fetch['BKU_URAIAN'];
	...
	<div style="height: 100%">
		<table width="100%">
			<tbody>
				<tr>
					<td colspan="9" style="font-size: 100%;">DINAS KOMUNIKASI DAN INFORMATIKA<br>KABUPATEN LEBAK</td>
				</tr>
				<tr>
	...
	<div style="height: 100%">
		...
		<table>
			<tr>
				<td style="text-align: left;">Yang bertandatangan dibawah ini Pengguna Anggaran/Kuasa Pengguna Anggaran Kegiatan '.$nama_kegiatan.' di '.$nama_organisasi.'.</td>
			<tr>
		</table>
		<table class="border" width="100%" style="border: 3px double black;">
			<thead>
...
/*	$mpdf->AddPage('P','','','','',10,10,10,10);
	...
	while ($fetch_r = mysql_fetch_array($result)) {
		...
		while ($fetch_rkp = mysql_fetch_array($result_rkp)) {
			...
			else $atas_nama = ' an. '.$fetch_rkp['BKU_ATAS_NAMA'];
			$data_rekap['uraian'] 	= $fetch_rkp['BKU_URAIAN_RO'].$atas_nama;
			$data_rekap['ls'] 		= '';
