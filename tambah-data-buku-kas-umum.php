...
if (isset($_POST['submit-tambah-bku'])) {
	...
	$uraian_ro = $_POST['BKU-URAIAN-RO'];
	$atas_nama = $_POST['BKU-ATASNAMA'];
	$penerimaan = intval(str_replace(".", "", $_POST['BKU-PENERIMAAN']));
	$pengeluaran = intval(str_replace(".", "", $_POST['BKU-PENGELUARAN']));
	$lampiran = $_POST['BKU-LAMPIRAN'];
	$pajak = intval(str_replace(".", "", $_POST['BKU-PAJAK-1']));
	$pajak = intval(str_replace(".", "", $_POST['BKU-PAJAK-2']));
	$pajak = intval(str_replace(".", "", $_POST['BKU-PAJAK-3']));
	$pajak = intval(str_replace(".", "", $_POST['BKU-PAJAK-4']));
	...
	if (!empty($_FILES["BKU-LAMPIRAN"])) {
		...
		if (!$success) { 
			...
			$query = "INSERT INTO TB_BUKU_KAS_UMUM(BKU_NO_URUT, LPJ_ID, BKU_TGL_SUB_KEG, BKU_KODE_REK, BKU_URAIAN, BKU_URAIAN_RO, BKU_ATAS_NAMA, BKU_PENERIMAAN, BKU_PENGELUARAN, BKU_PAJAK, BKU_LAMPIRAN)
					  VALUES($no_urut, $lpj_id, '$tgl', '$kodering', '$uraian', '$uraian_ro', '$atas_nama', '$penerimaan', '$pengeluaran', '$pajak', '$name')";
		...
		$query = "INSERT INTO TB_BUKU_KAS_UMUM(BKU_NO_URUT, LPJ_ID, BKU_TGL_SUB_KEG, BKU_KODE_REK, BKU_URAIAN, BKU_URAIAN_RO, BKU_ATAS_NAMA, BKU_PENERIMAAN, BKU_PENGELUARAN, BKU_PAJAK)
				  VALUES($no_urut, $lpj_id, '$tgl', '$kodering', '$uraian', '$uraian_ro', '$atas_nama', '$penerimaan', '$pengeluaran', '$pajak')";
