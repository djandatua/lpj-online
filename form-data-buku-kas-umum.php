<?php
	...
	$query       = "SELECT * FROM TB_M_URAIAN WHERE KODE_REK";
...
<div class='sub-content' id='data-utama-lpj'>
	...
	<?php if ($allowedit) { ?>
	<div class='subtitle'>
		<div class='title'><h2>Input Data Buku Kas Umum & Rincian Objek Belanja</h2></div>
	</div>
			...
			<form id='input-bku' name='input-bku' action='tambah-data-buku-kas-umum.php' method='POST' enctype='multipart/form-data'>
				<table>
					<tbody>
						...
						<tr>
							<td style='width: 200px;'><label for='BKU-URAIAN'>Uraian Buku Kas Umum</label></td>
							<td>: <input id='BKU-URAIAN' name='BKU-URAIAN' type='text' value='' style='width:400px;' /></td>
						</tr>
						<tr>
							<td style='width: 200px;'><label for='BKU-URAIAN-RO'>Uraian Rincian Objek</label></td>
							<td>: <input id='BKU-URAIAN-RO' name='BKU-URAIAN-RO' type='text' value='' style='width:400px;' /></td>
						</tr>
						<tr>
							<td style='width: 200px;'><label for='BKU-ATASNAMA'>Atas Nama</label></td>
							<td>: <input id='BKU-ATASNAMA' name='BKU-ATASNAMA' type='text' value='' style='width:400px;' /></td>
						</tr>
						<tr>
			...
			<table id='table' class='table data'>
				...
				while ($val = mysql_fetch_array($result)) {
					if (!empty($val['BKU_ATAS_NAMA'])) {
						...
						if (!empty($nip)) {
							$fetch = mysql_fetch_array(mysql_query("SELECT BKU_ATAS_NAMA FROM TB_BUKU_KAS_UMUM"));
							$nama_peg = $fetch['BKU_ATAS_NAMA'];
						}
					}
			?>
					<tr>
...
<div class='subtitle'>
		<div class='title'><h2>Tabel Data Buku Rincian Objek Belanja</h2></div>
	</div>
	<div class='sub-container'>
		<div class='cust-register'>
		<?php
		...
		?>
			<table id='table' class='table data'>
				<thead>
					<tr>
						...
						<th rowspan="2" width="52%">Atas Nama</th>
						<th colspan="3" width="25%">Pengeluaran</th>
					</tr>
					<tr>
						<th width="30%">LS/UP/GU/TU</th>
					</tr>
			...
			?>
					<tr>
						<td><center><?php echo $val['BKU_NO_URUT'] ?></center></td>
						<td><?php echo date_format(date_create($val['BKU_TGL_SUB_KEG']), 'd/m/Y') ?></td>
						<td><?php echo $val['BKU_URAIAN_RO'] ?></td>
						<td><?php if (!empty($val['BKU_ATAS_NAMA'])) echo $nama_peg ?></td>
						<td style='text-align: center;'><?php if ($val['BKU_PENGELUARAN'] != NULL && $val['BKU_PENGELUARAN'] != 0) echo number_format($val['BKU_PENGELUARAN'], 0, ',', '.'); $jml_pengeluaran += $val['BKU_PENGELUARAN'] ?></td>
					</tr>
			<?php
					if (!empty($val['BKU_PAJAK'])) {
						$pajak = explode("|", $val['BKU_PAJAK']);
			...
			?>
				...
				<tr>
						...
						<th style='text-align: center;'><b><?php echo number_format($jml_pengeluaran, 0, ',', '.') ?></b></th>
					</tr>
				</tbody>
			</table>
		<?php
